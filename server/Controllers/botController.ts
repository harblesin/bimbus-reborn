...
import db from "../Config/dbConfig"; // 👈 changed line
...
const addYoutubeLink = async (req: any, res: any) => {
  ...
  try {
    ...
    let values = [
      linkInfo.player_response.videoDetails.title,
      link,
      linkInfo.player_response.videoDetails.thumbnail.thumbnails[0].url,
    ];
...
const updateOrder = async (req: any, res: any) => {
  ...
  list.forEach((l: any, i: any) => {
    caseString = caseString + ` WHEN id = ${l.id} THEN ${i + 1} `;
  });
